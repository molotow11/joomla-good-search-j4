...
$path = Joomla\CMS\Helper\ModuleHelper::getLayoutPath('mod_articles_good_search', $params->get('module_template', $params->get('module_template', 'Default') . '') . '/template');
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		$("#GSearch<?php echo $module->id; ?>").find("select").each(function(k, el) {
			$(el)[0].ChoicesJS = new Choices(el, {
				shouldSort: false,
...
?>
		...
		<?php if((JFactory::getApplication()->input->getWord("option") == "com_j2store"
					&& JFactory::getApplication()->input->getWord("view") == "producttags"
					&& JFactory::getApplication()->input->getWord("tag") != "")
					|| JFactory::getApplication()->input->getWord("j2store_tag")
			) {
			//j2store automatically restriction by tag
			$tagId = JFactory::getDBO()->setQuery("SELECT id FROM #__tags WHERE alias = '". JFactory::getApplication()->input->getWord("tag") ."'")->loadResult();
		?>
		<input type="hidden" name="j2store_tag" value="<?php echo JFactory::getApplication()->input->getWord("j2store_tag", $tagId); ?>" /> 
		<?php
			}
		?>
